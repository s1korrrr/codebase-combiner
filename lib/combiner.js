...
function validateGlobPattern(pattern) {
  ...
  for (const expandedPattern of braceExpand(pattern)) {
    if (expandedPattern.split('/').filter((segment) => segment === '**').length > 2) {
      throw new Error('Each glob pattern may contain at most two globstar segments.');
    }
  }
}
...
function parseRunFilterInput(text, kind) {
  if (kind === 'glob') {
    return String(text)
      .split(/[\n,]/)
  ...
  if (kind === 'extension') {
    return String(text)
      .split(/[\s,;|]+/)
      .map((value) => stripDot(value.trim().toLowerCase()))
      .filter(Boolean);
  }
  throw new TypeError(`Unsupported run-filter kind: ${kind}`);
}
...
async function collectFiles(
  ...
  if (rootMetadata.isSymbolicLink()) {
    const error = new Error('Symbolic-link workspace roots are not supported.');
    error.name = 'RootSymlinkError';
    throw error;
  }
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (readResult.status !== 'accepted') {
          skipSummary[readResult.status] += 1;
          continue;
        }
        ...
        skipSummary.symbolicLink += 1;
      } else {
        skipSummary.unreadable += 1;
      }
    }
  }
  ...
  Object.defineProperty(results, 'skipSummary', {
    value: Object.freeze({ ...skipSummary }),
    enumerable: false,
  });
  return results;
}
...
async function readWorkspaceFile(absolutePath, canonicalRoot, maximumBytes) {
  ...
  try {
    ...
    if (!openedMetadata.isFile() || openedMetadata.size < 0n) return { status: 'unreadable' };
    if (openedMetadata.size > BigInt(maximumBytes)) return { status: 'oversized' };

    const canonicalPath = await fs.promises.realpath(absolutePath);
    if (!isWithinRoot(canonicalRoot, canonicalPath)) return { status: 'symbolicLink' };
    ...
    if (totalBytes > maximumBytes) return { status: 'oversized' };
    return { status: 'accepted', buffer: Buffer.concat(chunks, totalBytes) };
  } catch (error) {
    return { status: error?.code === 'ELOOP' ? 'symbolicLink' : 'unreadable' };
  } finally {
    await handle?.close();
  }
}
...
function renderBlock(file, outputFormat) {
  ...
  return `// File: ${sanitizeDisplayPath(file.relativePath)}\n${file.content}\n\n`;
}
